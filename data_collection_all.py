...
def pid_controller(pos, vel, att, ang_vel, posd, attd, dhat, jifen, dt, t):
    """PID controller function adapted from run_in_airsim_pid.py"""
    x = pos[0][-1]
    y = pos[1][-1]
    z = pos[2][-1]
...
def nn_adaptive_controller(pos, vel, att, ang_vel, posd, attd, dhat, jifen, dt, t, neural_net=None, optimizer=None):
    """Neural Network Adaptive controller function adapted from run_in_airsim_nnadaptive.py"""
    x = pos[0][-1]
    y = pos[1][-1]
    z = pos[2][-1]
    u = vel[0][-1]  # Use direct velocity from AirSim
    ...
    if neural_net is not None:
        ...
        if optimizer is not None and len(pos[0]) > 5:  # Update only after enough data
            neural_net.train()
            optimizer.zero_grad()
...
class SimpleFlightController:
    def __init__(self):
        ...
        # Neural network setup for nnadaptive method
    ...
    def run_simulation(self, total_time=60.0, trajectory_func=None, profile=None,
        ...
        # Initialize neural network for nnadaptive method
        if method == "nnadaptive":
            self.neural_net = AdaptiveNeuralNetwork(input_dim=12, hidden_dim=64, output_dim=3)
            self.neural_net.enable_adaptation()
            self.optimizer = torch.optim.Adam(self.neural_net.adaptive_layer.parameters(), lr=0.001)
        ...
        while self.simulation_time < total_time:
            ...
            if len(self.pos_history[0]) >= 3:
                # Select controller based on method parameter
                if method == "pid":
                    thr, r_des, p_des, y_des, self.dhat, self.jifen = pid_controller(
...
def main():
    ...
    def run_one(profile, condition, traj_fn, traj_type, sim_time, method="adaptive", out_root=None):
        print(f"[Run] {traj_type} | {condition} | profile={profile['tag']} | method={method} | {sim_time}s")
        # 每次运行前：强制复位到起飞原点
        reset_to_home()
        # 新建控制器并执行
    ...
    if args.mode and args.idx is not None:
        ...
        if args.mode == 'train':
            ...
            run_one(items[i][0], items[i][1], test3_random_spline_trajectory(), "random", 60.0, args.method)
        else:
            items = test_list
            print("可选测试风况：")
            for i,(p,c) in enumerate(items): print(f"[{i}] {p['tag']} -> {c}")
        ...
        if args.mode == 'train':
            print(f"运行所有训练风况 ({len(train_list)} 个) 使用 {args.method} 方法:")
